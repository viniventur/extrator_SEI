...
def main():
    ...
    # Analisar procs da unidade?
    check_proc_unidade = st.checkbox('Selecionar processos abertos na unidade.', False)

    # Lista de Processos
    if not check_proc_unidade:
    ...
    if buscar:
        ...
        with st.spinner('Buscando dados, aguarde...'):
            ...
            if docs:
                    ...
                    if check_proc_unidade:

                        processos_tratados = procs_unidade(unidade)

                    else:

                        if lista_processos:
    
                            processos_tratados, total_linhas, linhas_validas = tratar_processos_input(lista_processos)
                    ...
                    df_processos = pd.DataFrame({"Processos": processos_tratados})
